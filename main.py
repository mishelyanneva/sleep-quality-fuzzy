...
def build_fuzzy_system():
    ...
    # Membership функции за температура
    temperature["cold"] = fuzz.trimf(temperature.universe, [10, 10, 18])
    temperature["comfortable"] = fuzz.trimf(temperature.universe, [17, 22, 27])
    temperature["hot"] = fuzz.trimf(temperature.universe, [26, 35, 35])
    ...
    # Fuzzy правила
    rule1 = ctrl.Rule(
        sleep_duration["long"] & noise["low"] & light["dark"] & temperature["comfortable"],
        sleep_quality["good"]
    )

    rule2 = ctrl.Rule(
        sleep_duration["normal"] & noise["low"] & light["dark"] & temperature["comfortable"],
        sleep_quality["good"]
    )

    rule3 = ctrl.Rule(
        sleep_duration["normal"] & noise["low"] & light["moderate"] & temperature["comfortable"],
        sleep_quality["good"]
    )

    rule4 = ctrl.Rule(
        sleep_duration["normal"] & noise["medium"] & light["moderate"] & temperature["comfortable"],
        sleep_quality["average"]
    )

    rule5 = ctrl.Rule(
        sleep_duration["short"],
        sleep_quality["poor"]
    )

    rule6 = ctrl.Rule(
        sleep_duration["short"] & noise["high"],
        sleep_quality["poor"]
    )

    rule7 = ctrl.Rule(
        sleep_duration["short"] & light["bright"],
        sleep_quality["poor"]
    )

    rule8 = ctrl.Rule(
        sleep_duration["long"] & noise["medium"] & light["dark"] & temperature["comfortable"],
        sleep_quality["average"]
    )

    rule9 = ctrl.Rule(
        sleep_duration["long"] & noise["high"] & light["dark"] & temperature["comfortable"],
        sleep_quality["average"]
    )

    rule10 = ctrl.Rule(
        sleep_duration["long"] & noise["low"] & light["bright"] & temperature["comfortable"],
        sleep_quality["average"]
    )

    rule11 = ctrl.Rule(
        sleep_duration["normal"] & noise["low"] & light["dark"] & temperature["cold"],
        sleep_quality["average"]
    )

    rule12 = ctrl.Rule(
        sleep_duration["long"] & noise["low"] & light["dark"] & temperature["hot"],
        sleep_quality["average"]
    )

    rule13 = ctrl.Rule(
        temperature["hot"] & noise["medium"] & light["bright"],
        sleep_quality["poor"]
    )

    rule14 = ctrl.Rule(
        noise["high"] & light["bright"],
        sleep_quality["poor"]
    )

    rule15 = ctrl.Rule(
        sleep_duration["normal"] & noise["low"] & light["dark"] & temperature["hot"],
        sleep_quality["average"]
    )
...
def run_simulation(control_system, sleep_duration_value, noise_value, light_value, temperature_value):
...
def plot_membership_functions(variables, output_path):
    ...
    configs = [
        ...
        ("Температура в помещението (°C)", variables["temperature"]),
...
def plot_result_example(variables, result, sample_inputs, output_path):
    ...
    info_text = (
        ...
        f"Температура: {sample_inputs['temperature']} °C\n"
...
def main():
    ...
    for case in test_cases:
        ...
        print(f"Температура в помещението: {case['temperature']} °C")
